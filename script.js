...
function save(){
  const payload = { items: state.items, selected: Array.from(state.selected) };
...
function load(){
  try {
    ...
    if(!Array.isArray(data.items) || data.items.length !== 25) return false;
...
function shuffleBoard(){
  ...
  while(base.length < 25) base.push("Case vide");
...
(function init(){
  const ok = load();
  if (!ok) { shuffleBoard(); } else { render(); }
